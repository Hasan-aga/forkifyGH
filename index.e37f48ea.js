...
var _searchValueJs = require("./views/searchValue.js");
var _searchValueJsDefault = parcelHelpers.interopDefault(_searchValueJs);
...
const controlRecipes = async function() {
    try {
        ...
        (0, _resultsViewJsDefault.default).update(_modelJs.getResultsOfPage());
...
const controlSearchResults = async function() {
    try {
        const query = (0, _searchValueJsDefault.default).query;
        if (!query) return;
        (0, _resultsViewJsDefault.default).renderSpinner();
        await _modelJs.loadSearchResults(query);
        console.log("model.state", _modelJs.state);
        const paginatedResults = _modelJs.getResultsOfPage();
        console.log("paginatedResults", paginatedResults);
        (0, _resultsViewJsDefault.default).render(paginatedResults);
        (0, _pagenationViewJs.paginationView).render(_modelJs.state.search);
    } catch (error) {
        console.error(`could not get results. ${error}`);
        (0, _resultsViewJsDefault.default).renderError(error.message);
    }
};
const controlPagination = function(destinationPage) {
    (0, _resultsViewJsDefault.default).render(_modelJs.getResultsOfPage(destinationPage));
    (0, _pagenationViewJs.paginationView).render(_modelJs.state.search);
};
const controlServings = function(changeAmount) {
    //update the ingriedients
    _modelJs.updateServings(changeAmount);
    //display new ingriedients
    // recipeView.render(model.state.recipe);
    (0, _recipeViewJsDefault.default).update(_modelJs.state.recipe);
};
const controlBookmark = function() {
    _modelJs.switchBookmark();
    (0, _recipeViewJsDefault.default).update(_modelJs.state.recipe);
    (0, _bookmarksViewJs.bookmarksView).render(_modelJs.state.bookmarks);
};
const controlUploadRecipe = async function(data) {
    try {
        const uploadedRecipe = await _modelJs.uploadRecipe(data);
        _modelJs.state.recipe = uploadedRecipe;
        window.location.hash = uploadedRecipe.id;
        controlBookmark();
        (0, _uploadRecipeViewJsDefault.default).switchFormAndOverlay();
    } catch (error) {
        (0, _uploadRecipeViewJsDefault.default).renderError(`Error, ${error}`);
    }
};
const init = function() {
    _modelJs.getBookmarksFromLocal();
    ...
    (0, _searchValueJsDefault.default).addSearchHandler(controlSearchResults);
    (0, _pagenationViewJs.paginationView).addHandlerPagination(controlPagination);
    (0, _uploadRecipeViewJsDefault.default).addShowRecipeFormHandler();
    (0, _uploadRecipeViewJsDefault.default).addUploadRecipeHandler(controlUploadRecipe);
};
init();

},{"core-js/modules/web.immediate.js":"49tUX","./model.js":"Y4A21","./views/recipeView.js":"l60JC","./views/searchValue.js":"QsGFe","./views/resultsView.js":"cSbZE","./views/pagenationView.js":"7Vqbs","./views/bookmarksView.js":"4Lqzq","./views/uploadRecipeView.js":"cc5Cn","regenerator-runtime/runtime":"dXNgZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"49tUX":[function(require,module,exports) {
...
parcelHelpers.export(exports, "getResultsOfPage", ()=>getResultsOfPage);
parcelHelpers.export(exports, "updateServings", ()=>updateServings);
parcelHelpers.export(exports, "switchBookmark", ()=>switchBookmark);
parcelHelpers.export(exports, "getBookmarksFromLocal", ()=>getBookmarksFromLocal);
...
const loadRecipe = async function(id) {
    try {
        ...
        if (state.bookmarks.some((bookmark)=>bookmark.id === id)) recipe = state.bookmarks.find((bookmarkedRecipe)=>bookmarkedRecipe.id === id);
        else {
            const data = await (0, _helper.ApiTools).getJson(`${(0, _config.API_URL)}${id}?key=${(0, _config.API_KEY)}`);
            recipe = data.data.recipe;
        }
        const alreadyBookmarked = this.state.bookmarks.find((bookmark)=>bookmark.id === recipe.id);
...
const loadSearchResults = async function(query) {
    try {
        const data = await (0, _helper.ApiTools).getJson(`${(0, _config.SEARCH_URL)}${query}&key=${(0, _config.API_KEY)}`);
        state.search.currentPage = 1;
        state.search.query = query;
        state.search.count = data.results;
        if (Array.isArray(data.data.recipes) && data.data.recipes.length === 0) throw new Error(`No results for ${query}`);
...
const getResultsOfPage = function(page = this.state.search.currentPage) {
    try {
        ...
        console.log("result per page", this.state.search.results.slice(startIndex, endIndex));
...
const updateServings = function(changeAmount) {
    const newServing = state.recipe.servings + changeAmount <= 0 ? 1 : state.recipe.servings + changeAmount;
    state.recipe.ingredients.forEach((ingredient)=>ingredient.quantity *= newServing / state.recipe.servings);
    state.recipe.servings = newServing;
};
const switchBookmark = function() {
    ...
    this.state.recipe.bookmark ? this.state.bookmarks.push(this.state.recipe) : this.state.bookmarks.splice(indexOfBookmark, 1);
    saveBookmarksToLocal();
    this.state.search.results.forEach((storedRecipe)=>{
        if (storedRecipe.id === this.state.recipe.id) storedRecipe.bookmark = storedRecipe.bookmark ? false : true;
    });
};
...
const getBookmarksFromLocal = function() {
    state.bookmarks = localStorage.getItem("bookmarks") === null ? [] : [
        ...JSON.parse(localStorage.getItem("bookmarks"))
    ];
};
const uploadRecipe = async function(newRecipe) {
    try {
        const ingredients1 = Object.entries(newRecipe).filter((ingPair)=>ingPair[0].startsWith("ingredient") && ingPair[1] !== "").map((ing)=>{
            const ingredients = ing[1].replaceAll(" ", "").split(",");
            if (ingredients.length < 3) throw new Error("Wrong ingriedients format, please use correct format: &ltquantity, unit, description&gt");
        ...
        const data = await (0, _helper.ApiTools).sendJson(`${(0, _config.API_URL)}?key=${(0, _config.API_KEY)}`, recipe);
        console.log(data);
...
const SEARCH_URL = "https://forkify-api.herokuapp.com/api/v2/recipes?search=";
const API_KEY = "184ef29a-7426-4a38-803f-faab2959ab92";
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "ApiTools", ()=>ApiTools);
var _config = require("./config");
class ApiTools {
    ...
    static getJson = async function(url) {
        try {
            const res = await Promise.race([
                fetch(url),
                ApiTools.timeout((0, _config.TIMEOUT_SECONDS)), 
            ]);
            const data = await res.json();
            if (!res.ok) throw new Error(`failed to get data: ${data.error}, ${res.status}`);
            return data;
        } catch (error) {
    ...
    static sendJson = async function(url, uploadData) {
        try {
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(`failed to send data: ${data.message}, ${res.status}`);
            return data;
        } catch (error) {
            console.log(error);
            throw error;
...
class RecipeView extends (0, _viewDefault.default) {
    ...
    <div class="recipe__details">
      ...
      <button class="btn--round btn--bookmark">
        <svg class="">
          <use href="${0, _iconsSvgDefault.default}.svg#icon-bookmark${this._data.bookmark ? "-fill" : ""}"></use>
        </svg>
      </button>
    </div>
...
},{"url:../../img/icons.svg":"loVOp","./view":"bWlJ9","fracty":"hJO4d","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"loVOp":[function(require,module,exports) {
...
class View {
   ...
   */ render(state) {
    ...
    update(state) {
        ...
        //get new markup
        const recipeHtml = this._generateMarkup();
        const newElements = Array.from(document.createRange().createContextualFragment(recipeHtml).querySelectorAll("*"));
        //get current rendered markup
        const renderedELements = Array.from(this._parentElement.querySelectorAll("*"));
        //compare the two
        renderedELements.forEach((renderedElement, index)=>{
            ...
            if (!renderedElement.isEqualNode(newElements[index]) && newElements[index].firstChild?.nodeValue.trim() !== "") renderedElement.textContent = newElements[index].textContent;
            // Updates changed ATTRIBUES
            if (!renderedElement.isEqualNode(newElements[index])) Array.from(newElements[index].attributes).forEach((attr)=>renderedElement.setAttribute(attr.name, attr.value));
        });
    }
}
...
},{}],"QsGFe":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
"use strict";
class SearchValue {
...
},{"./view":"bWlJ9","url:../../img/icons.svg":"loVOp","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"7Vqbs":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class PaginationView extends (0, _viewDefault.default) {
    ...
    _generateMarkup() {
        ...
        if (this._data.currentPage === 1 && this._data.getTotalNumberOfPages() > 1) return ` 
      ...
      <svg class="search__icon">
        ...
        if (this._data.currentPage === 1 && this._data.getTotalNumberOfPages() === 1) return "";
        // at last page
        if (this._data.currentPage === this._data.getTotalNumberOfPages()) return `
      <button data-goto="${this._data.getTotalNumberOfPages() - 1}" class="btn--inline pagination__btn--prev">
      <svg class="search__icon">
...
class BookmarksView extends (0, _viewDefault.default) {
    ...
    _generateMarkup() {
        const id = window.location.hash.slice(1);
        return this._data.length > 0 ? this._data.map((bookmark)=>`
    <li class="preview">
...
},{"./view":"bWlJ9","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cc5Cn":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _iconsSvg = require("url:../../img/icons.svg");
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
...
var _fractyDefault = parcelHelpers.interopDefault(_fracty);
"use strict";
class UploadRecipeView extends (0, _viewDefault.default) {
    ...
    addShowRecipeFormHandler() {
        [
            this._addRecipeButton,
            this._closeRecipeButton,
            this._overlay
        ].forEach((ev)=>ev.addEventListener("click", this.switchFormAndOverlay.bind(this)));
    }
...
},{"url:../../img/icons.svg":"loVOp","./view":"bWlJ9","fracty":"hJO4d","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dXNgZ":[function(require,module,exports) {
/**
